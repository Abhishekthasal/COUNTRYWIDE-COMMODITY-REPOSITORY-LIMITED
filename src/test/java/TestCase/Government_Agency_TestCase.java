...
//@Test
	void Government_Agency_Deposite_Request_Maker() throws IOException {
		try {
			...
			test.log(LogStatus.FAIL, test.addScreenCapture(ScreenShort.CaptureScreen(driver)) + "Test failed");
		}
		Report.endTest(test);
		Report.flush();
	}
...
//@Test
	...
	void DashBoard_WareHouse_Checker() throws IOException {
		try {
			...
			test.log(LogStatus.PASS, "Government_Agency_WareHouse_Checker is successfull");
		} catch (Exception e) {
			test.log(LogStatus.FAIL,
					test.addScreenCapture(ScreenShort.CaptureScreen(driver)) + "Test failed: " + e.getMessage());
		}
